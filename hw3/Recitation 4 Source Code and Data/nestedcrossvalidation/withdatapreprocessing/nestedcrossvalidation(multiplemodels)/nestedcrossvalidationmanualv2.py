...
for train_indices, test_indices in outer_cross_validation.split(dataset, labels):
    ...
    for inner_train_indices, inner_test_indices in inner_cross_validation.split(current_training_part, current_training_part_label):
        ...
        for C in svm_parameter_grid["C"]:
            for kernel in svm_parameter_grid["kernel"]:
                ...
                svm.fit(scaled_inner_training_dataset, inner_training_label)

                predicted = svm.predict(scaled_inner_test_dataset)
                if (C, kernel) not in svm_performance:
                    svm_performance[(C, kernel)] = []
                svm_performance[(C, kernel)].append(f1_score(inner_test_label, predicted, average="micro"))


        # TODO: What if an ML method has a randomization component... e.g, MLP, random forest
    ...
    knn_with_best_param.fit(scaled_current_training_part, current_training_part_label)
    knn_predicted = knn_with_best_param.predict(scaled_current_test_part)
    ...
    svm_with_best_param.fit(scaled_current_training_part, current_training_part_label)
    svm_predicted = svm_with_best_param.predict(scaled_current_test_part)
